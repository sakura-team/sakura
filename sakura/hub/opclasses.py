...
OpClsInfo = namedtuple('OpClsInfo', ['cls_id', 'daemon_id', 'name', 'tags', 'icon', 'nb_inputs', 'nb_outputs'])
...
class OpClassRegistry(object):
    ...
    def store(self, daemon_id, name, tags, icon, nb_inputs, nb_outputs):
        if (daemon_id, name) in self.info_per_daemon_and_name:
            ...
            self.next_cls_id += 1
        desc = OpClsInfo(cls_id, daemon_id, name, tags, icon, nb_inputs, nb_outputs)
