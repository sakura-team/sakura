...
class Operator:
    ...
    def __init__(self, op_id, hub_events_recorder, op_dir):
        self.op_id = op_id
        self.root_dir = op_dir
        self.event_lock = Semaphore()
        self.hub_events_recorder = hub_events_recorder
        self.hub_events = []
        self.flushing_hub_events = False
    ...
    def flush_hub_events(self):
        # retrieve events and reinit for next time
        all_events = self.hub_events
        self.hub_events = []
        self.flushing_hub_events = False
        # drop duplicated events, keep only the last one
        seen_events = set()
        events = []
        for evt, args, kwargs in reversed(all_events):
            hashable_event = (evt, args, tuple(kwargs.items()))
            if hashable_event not in seen_events:
                events.append((evt, args, kwargs))
                seen_events.add(hashable_event)
        # publish to hub
        self.hub_events_recorder(tuple(reversed(events)))
    def push_event_to_hub(self, evt, *args, **kwargs):
        if not self.check_mode:
            self.hub_events.append((evt, args, kwargs))
            if not self.flushing_hub_events:
                self.flushing_hub_events = True
                gevent.spawn(self.flush_hub_events)
    ...
    def move(self):
        if self.is_ready():
            self.push_event_to_hub('hub:check_move')
    def get_plug_origins(self):
