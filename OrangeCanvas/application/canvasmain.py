...
###UNDERTRACKS###START              
from .UTlogin       import UTLoginDialog
...
###UNDERTRACKS###START 
        self.help_dock_UT = DockableWindow(self.tr("UT Help"), self,
                                        objectName="help-dock-UT")
        self.help_dock_UT.setAllowedAreas(Qt.NoDockWidgetArea)
        self.help_view_UT = QWebView()
###UNDERTRACKS###END
        ...
        cache.setCacheDirectory(os.path.join(config.cache_dir(), "help", "help-view-cache"))
        manager.setCache(cache)
        manager.sslErrors.connect(self.sslErrorHandler)
        self.help_dock.setWidget(self.help_view)
        self.help_dock.hide()

###UNDERTRACKS###START 
        self.help_dock_UT.setWidget(self.help_view_UT)
        self.help_dock_UT.hide()
###UNDERTRACKS###END

        self.setMinimumSize(600, 500)

    def sslErrorHandler(self,reply,errors_list):
...
###UNDERTRACKS###START		
        self.log_on_UT_action = \
            QAction(self.tr("Log on UT ... "), self,
                    objectName="action-log-on-UT",
                    toolTip=self.tr("Log on UnderTracks for opening/saving process, or loading/saving data."),
                    triggered=self.log_on_UT,
                    )

        self.open_from_UT_action = \
            QAction(self.tr("Open from UT"), self,
                    objectName="action-open-from-UT",
                    toolTip=self.tr("Open a scheme from UnderTracks Database, erasing the current one"),
                    triggered=self.open_scheme_from_UT,
                    )

        self.save_on_UT_action = \
            QAction(self.tr("Save On UT"), self,
                    objectName="action-save-on-UT",
                    toolTip=self.tr("Save current scheme on UnderTracks Database."),
                    triggered=self.save_scheme_on_UT,
                    )
...
###UNDERTRACKS###END
    ...
    def setup_menu(self):
        ...
        file_menu.addSeparator()
###UNDERTRACKS###START
        file_menu.addAction(self.log_on_UT_action)
        file_menu.addAction(self.open_from_UT_action)
        file_menu.addAction(self.save_on_UT_action)
        file_menu.addSeparator()
...
###UNDERTRACKS###START
    def log_on_UT(self):
        """Connection to the UnderTracks Database. Return
        QDialog.Accepted if the connection is established
        QDialog.Rejected if not.

        """
        
        dialog = UTLoginDialog(self)
        dialog.setWindowTitle(self.tr("UnderTracks Login"))
        dialog.setFixedSize(300, 240)
        settings = QSettings()
        
        dialog.setLogin(unicode(settings.value("ut-login/login","unkown", type=unicode)))

        done = False
        while not done:
            status = dialog.exec_()
            if status == QDialog.Accepted:
                login = dialog.editor.login()
                password = dialog.editor.password()
                url     = "https://undertracks.imag.fr/scripts/OrangeScripts/ExistingUser.php"
                data    = _u.urlencode({"login":login,"pass":password})
                req     = _u2.Request(url,data)
                f       = _u2.urlopen(req)
                res = f.read()
                if "Done" in res:
                    settings.setValue("ut-login/login",login)
                    settings.setValue("ut-login/connected",True)
                    
                    message_information("Connection Ok! Your login is saved in Orange.")
                    done = "True"
                else:
                    message_critical("Connection is rejected!\nLogin is not known or password is wrong.")
                    settings.setValue("ut-login/login","unknown")
                    settings.setValue("ut-login/connected",False)
                    done = False
            else:
                done = True 
    ...
    def save_scheme_on_UT(self):
        """Save the current scheme on the UnderTracks Database. Return
        QDialog.Accepted if the scheme was successfully saved and
        QDialog.Rejected if the user is not logged.
        ...
        while not done:
            ...
            if status == QDialog.Accepted:
                ...
                # send the scheme to UT
                document = self.current_document()
                curr_scheme = document.scheme()
                title = curr_scheme.title
                description = curr_scheme.description 
                login = unicode(settings.value("ut-login/login","unknown", type=unicode))
                ...
                if res == "Done":
                    ...
                    selected = message_question("This process "+title+" already exist!",
                                                self.tr("Save Anyway ?"),
                                                self.tr("Save Anyway ?"),
                                                buttons=QMessageBox.Save | QMessageBox.Cancel,
                                                default_button=QMessageBox.Save,
    ...
    def open_scheme_from_UT(self):
        """Open a scheme from the UnderTracks Database. Return
        QDialog.Accepted if the scheme was successfully loaded and
        QDialog.Rejected if the user is not logged.

        """
        # 1 - Recovering the list of Schemes from UnderTracks
        settings = QSettings()
        login = unicode(settings.value("ut-login/login","unknown", type=unicode))
        url     = "https://undertracks.imag.fr/scripts/OrangeScripts/OrangeProcesses.php"
        data    = _u.urlencode({"login":login})
        req     = _u2.Request(url,data)
        f       = _u2.urlopen(req)
        res = f.read()
        ...
        dialog = UTOpenDialog(self)
        dialog.setWindowTitle(self.tr("Open a Process from UnderTracks"))
        dialog.setSchemeList(scheme_names,scheme_descs)
        
        status = dialog.exec_()
        
        if status == QDialog.Accepted:
            scheme_name = scheme_names[dialog.editor.get_scheme()]
            login = unicode(settings.value("ut-login/login","unknown", type=unicode))
            url     = "https://undertracks.imag.fr/scripts/OrangeScripts/OpenOrangeProcess.php"
            data    = _u.urlencode({"login":login,"name":scheme_name})
            req     = _u2.Request(url,data)
            f       = _u2.urlopen(req)
            res = f.read()
            
            # We create a file, and then read it
            if sys.platform == "darwin":
                path = "/tmp/tmpScheme.ows"
            else:
    ...
    def widget_path_UT(self):
        # Open the Dialog box
        dialog = UTWidgetPath(self)
        status = dialog.exec_()
        
        return QDialog.Accepted
        
    def update_UT_widgets(self):
        """Update UT widgets. Return
        QDialog.Accepted if the widget(s) is(are) successfully updated and
        QDialog.Rejected if the user is not logged.
        """
        
        # Open the Dialog box
        dialog = UTUpdateDialog(self)
        status = dialog.exec_()
...
###UNDERTRACKS###START
                ...
                n = ref.split("OWUT")
                url = QUrl("https://undertracks.imag.fr/php/operators/operator_pass_view.php/"+n[-1])
                if not url:
                    message_information("Sorry, this UT widget does not contain documentation yet.",parent=self)
                else:
                    self.show_help(url)                                        
                return True
###UNDERTRACKS###END
...
###UNDERTRACKS###START 
            self.help_dock_UT.setFloatingWindowFlags(Qt.Tool)
###UNDERTRACKS###END
        else:
            self.help_dock.setFloatingWindowFlags(Qt.Window)
###UNDERTRACKS###START 
            self.help_dock_UT.setFloatingWindowFlags(Qt.Window)
###UNDERTRACKS###END
        ...
        if dockable:
            self.help_dock.setAllowedAreas(Qt.LeftDockWidgetArea | \
                                           Qt.RightDockWidgetArea)
###UNDERTRACKS###START 
            self.help_dock_UT.setAllowedAreas(Qt.LeftDockWidgetArea | \
                                           Qt.RightDockWidgetArea)
###UNDERTRACKS###END
        else:
            self.help_dock.setAllowedAreas(Qt.NoDockWidgetArea)
