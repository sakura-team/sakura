...
class UTUpdateDialog(QDialog):
    ...
    def __setupUi(self):
        ...
        url     = "https://undertracks.imag.fr/scripts/OrangeScripts/Widgets/ExistingWidgets_last.php"
    ...
    def update(self):
        ...
        for i in range(len(self.widgets_checkboxes)):
            if self.widgets_checkboxes[i].checkState() == 2:
                ...
                url         = "https://undertracks.imag.fr/scripts/OrangeScripts/Widgets/GiveWidget_last.php?category="+category+"&sub_category="+sub_category+"&widget="+name
                req         = _u2.Request(url)
                f           = _u2.urlopen(req)
                
                str_url = f.read()                
                widget = _u2.urlopen(str_url)
                zfile = _zfile.ZipFile(StringIO(widget.read()))
