#!/usr/bin/python
#Orange Widget developped by Denis B. Feb, 2016 from Succ.


"""
<name>Pred</name>
<icon>OWUTPred_icons/OWUTPred.svg</icon>
<description>Add an extra-attribute with the previous value of a given attribute</description>
...
class OWUTPred(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None):
        ...
        self.outputs = [("Pred Table", Orange.data.Table)]
        settings = QSettings()
    ...
    def compute(self):
        if self.data != None:
            res = mainOWUTPred(owutChoixAttribut(self,"configMainAttribute"), owutChoixAttribut(self,"configGroupByAttribute"), self.cfgKeepAtt1,  self.cfgKeepAtt2, str(self.cfgNameNewAtt1), self.data)
            if res!=None:
                owutSynchroniseChoixAttribut(self)
                self.saveSettings()
                self.send("Pred Table", res)
...
def mainOWUTPred(mainAttribute, groupByAttribute, pKeepAttribute1, pKeepAttribute2, pNameNewAtt1, dataIn):
    ...
    for i in range(len(dataIn)):
        if (i>0) and (dataIn[i][indGroupByAttribute].value==dataIn[i-1][indGroupByAttribute].value):
            resultat[i].append(dataIn[i-1][indMainAttribute].value)
        else:
            resultat[i].append(emptyPredecessor)
