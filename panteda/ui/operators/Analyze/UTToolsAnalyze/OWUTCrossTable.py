#!/usr/bin/python
#Orange Widget developped by Michael ORTEGA - 18/March/2014, puis maintenu par Denis B. Oct. 14, fev. 16

"""
<name>CrossTable</name>
<icon>OWUTCrossTable_icons/OWUTCrossTable.svg</icon>
<description>Compute the cross-table of an attribute (frequency distribution)</description>
<priority>41</priority>
"""

#todo:ajouter une sauvegarde des parametres
...
class OWUTCrossTable(OWWidget):
    settingsList = ['col1','col2','type','configExplanation','cfgNameNewAtt1']
    ...
    def __init__(self, parent=None, signalManager=None):
        ...
        if self.login != "unknown":
            ...
            owutAjouteChoixAttribut(boxSettings,self,"type","Type")
            OWGUI.button(self.controlArea, self, 'Apply', callback=self.compute)
            self.configExplanation = "Your comment/explanation."
            OWGUI.lineEdit(self.controlArea, self, "configExplanation", box="Any comment or explanation about your cross-table?")
    ...
    def dataset(self, data, id=None):
        ...
        if self.data != None:
            ...
            _type = owutInitChoixAttributEtVerifieConfig(self,"type",["PRow","PColumn","PTotal","Count","Chi2"])
            if _col1 and _col2 and _type:
                self.compute()
    ...
    def compute(self):
        if self.data != None:
            ...
            if (_col1 != _col2):
                res = mainOWUTCrossTable(owutChoixAttribut(self,"type"),_col1,_col2, str(self.cfgNameNewAtt1),self.data)
                if res!=None:
...
def mainOWUTCrossTable(t,n1,n2,pNameForNewAttribute,data):
    ...
    if t == "PRow":
        for i in range(len(variable1)):
            for j in range(len(variable2)):
                res[i][j+1] = str(float(str(res[i][j+1]))*100./float(nb_variable1[i]))
    
    elif t == "PColumn":
        for i in range(len(variable1)):
            for j in range(len(variable2)):
                res[i][j+1] = str(float(str(res[i][j+1]))*100./float(nb_variable2[j]))
    elif t == "Count":
        t=t #rien a faire tout est deja fait
    elif t == "PTotal":
        for i in range(len(variable1)):
            for j in range(len(variable2)):
                res[i][j+1] = str(float(str(res[i][j+1]))*100./float(nb_total))
    else:
        numerateur_chi2 = 0
